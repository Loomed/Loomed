...
public class IndexController {
	...
	public String getIndex(HttpServletRequest request, HttpSession session) {
		...
		//int LoginId = 0;
		//String LoginPass = "";
		List<Maps> LoginRoom = new ArrayList<>();

		/*
		 * // 入力されたidがintかどうか判定 log.info(LogEnum.IF.getLogValue() +
		 * "IndexService.isNum(id)"); if (IndexService.isNum(id)) {
		 * log.info(LogEnum.TRUE.getLogValue() + "IndexService.isNum(id)");
		 * LoginId = Integer.parseInt(id); } else {
		 * log.info(LogEnum.FALSE.getLogValue() + "/login"); return "/login"; }
		 *
		 * // 入力されたpassが0文字以上か判定 log.info(LogEnum.IF.getLogValue() +
		 * "pass.length() > 0"); if (pass.length() > 0) {
		 * log.info(LogEnum.TRUE.getLogValue() + "LoginPass == pass"); LoginPass
		 * = pass; } else { log.info(LogEnum.FALSE.getLogValue() + "/login");
		 * return "/login"; }
		 */
